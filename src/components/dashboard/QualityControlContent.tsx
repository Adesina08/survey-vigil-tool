...
export const QualityControlContent = ({ dashboardData, selectedLga, onFilterChange }: QualityControlContentProps) => {
  ...
  const {
    ...
    const summary = {
      ...
      approvalRate: totalSubmissions > 0 ? Number(((0 / totalSubmissions) * 100).toFixed(1)) : 0,
      notApprovedSubmissions: 0,
      notApprovedRate: totalSubmissions > 0 ? Number(((0 / totalSubmissions) * 100).toFixed(1)) : 0,
    ...
    return {
      summary,
      quotaProgress,
      statusBreakdown: { approved: 0, notApproved: 0 },
      productivity,
      errorBreakdown,
      achievementsByInterviewer,
      achievementsByLGA,
