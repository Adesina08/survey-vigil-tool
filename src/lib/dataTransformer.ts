...
/**
 * Extract all error codes/flags from a row and normalise them
 */
function extractErrorTypes(row: RawSurveyRow): ErrorTypeInfo[] {
  const errors = new Map<string, ErrorTypeInfo>();

  Object.keys(row).forEach((key) => {
    if ((key.startsWith("QC_FLAG_") || key.startsWith("QC_WARN_")) && isTruthy(row[key])) {
      const info = normaliseErrorType(key);
      if (!errors.has(info.slug)) {
        errors.set(info.slug, info);
      }
    }
  });
  ...
  return rawData
    ...
    .map((row) => {
      ...
      return {
        ...
        lga,
        state,
        errorTypes: extractErrorTypes(row).map((info) => info.label),
        timestamp: (getRowTimestamp(row) ?? new Date()).toISOString(),
